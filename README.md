...
- 卡片中的各种标签的含义：
  ...
  - 右上角的 `反馈`：反馈按钮，点击即可反馈。
  - 单词左边及例句右边的 `▶️`：点击播放音频。如果该卡片没有本地音频，则按钮会变为紫色，此时调用的是在线 TTS。
  - 单词右边的 `🔍`：点击跳转外部词典或网页查词，可参考下面的“高级选项”进行自定义。
  - 单词和假名右上角的 `②`：表示该单词音调模式的标记。
  - `［他動1］`：表示该单词的词性。  
  
    对于动词来说， `1` 为 “五段動詞”、 `2` 为“上一段・下一段動詞”、 `3` 为“カ変・サ変動詞”。
...
- 默认情况下，仅对没有本地音频的例句启用在线 TTS。 如果你想为所有例句启用，请注释掉 `function setEdgeTTS()` 中的以下代码：
